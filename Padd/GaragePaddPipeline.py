...
from .models.PowerPaintModel import PowerPaintModel

class GarageObjectAdder:
    def __init__(self, device: str = "cuda", garage_model = None) -> None:
        """
        ...
        self.augmenter = PowerPaintModel(device=self.device)
    ...
    def resize_and_random_flip(self, image: Image.Image, 
        ...
        max_factor = 2.5
        if new_size[0] > image.width / max_factor:
            new_size = (int(image.width / max_factor) , int(new_size[1] * image.width / max_factor / new_size[0] ) )
        elif new_size[1] > image.height / max_factor:
            new_size = ( int(new_size[0] * image.height / max_factor / new_size[1] ) , int(image.height / max_factor))
    ...
    def preprocess_scene_image(self, scene: Image.Image) -> Tuple[float, float, float, float]:
        ...
        """
        return [0,0,scene.size[0], scene.size[1]]     
    ...
    def generate_new_object(
        ...
        # Mask to bounded mask
        object_mask , _ = self.mask_to_box(object_mask)
        ...
        for i in range(10):
            ...
            # Base part of code
            sampled_coord = self.sample_random_coordinates(location_array)[0]
            ...
            _, bottom_point = self.calculate_average_depth_and_bottom_point(resized_object_depth, resized_object_mask)
            ...
            # Old size
            mask = mask.convert('L')
            old_mask_size = np.count_nonzero(np.array(mask))
            # mask.save(f'./augmentations/test/image_{i}.png')
            # Fill mask with last masks
            for bbox in boxes:
                mask = self.fill_image(mask , bbox )
            # mask.save(f'./augmentations/test/filled_image_{i}.png')
            
            mask = mask.convert('L')
            new_mask_size = np.count_nonzero(np.array(mask))
            # print(old_mask_size , new_mask_size)
            if new_mask_size < old_mask_size*0.5:
                good_sample = False
                continue
            else :
                good_sample = True
        ...
        result_for_point = self.PhysicsModel.run_inference(np.asarray(scene_image), np.asarray(object_image), (sampled_coord[0], sampled_coord[1]))
        if not result_for_point or not good_sample:
            print('not results for point')
            return [],[],[]

        input_image = {'image': scene_image, 'mask': mask}
        new_image = self.augmenter(
        ...
        return new_image, mask, prompt
    
    def mask_to_box(self, mask_image):
        ...
        bounded_mask = np.zeros_like(mask_image)
        bounded_mask[x_min:x_max + 1, y_min: y_max + 1] = 1
        # Преобразуем к типу Image
        bounded_mask = Image.fromarray(np.uint8(np.array(bounded_mask)*255))
        
        bbox = [x_min, y_min, x_max + 1, y_max + 1]
        return bounded_mask, bbox
    
    def calculate_iou(self, bbox1, bbox2):
        # Распаковываем координаты bbox
        ...
        if x_intersect_min < x_intersect_max and y_intersect_min < y_intersect_max:
            # Площадь пересечения
            area_intersect = (x_intersect_max - x_intersect_min) * (y_intersect_max - y_intersect_min)
        else:
            area_intersect = 0  # Нет пересечения

        # Площадь первого bbox
        area_bbox1 = (x1_max - x1_min) * (y1_max - y1_min)

        # Доля пересечения относительно первого bbox
        ratio_bbox1 = area_intersect / area_bbox1 if area_bbox1 > 0 else 0

        return ratio_bbox1
    
    def fill_image(self, image, bbox):
        black_rectangle = Image.new('RGB', (bbox[2] - bbox[0], bbox[3] - bbox[1]), (0, 0, 0))
        image.paste(black_rectangle, (bbox[0], bbox[1]))
        return image
    def volume_bbox(self, bbox):
    ...
    def __call__(
        ...
        for object_image, object_mask, prompt in zip(object_images, object_masks, prompts):
            result = self.generate_new_object(
            ...
            scene, mask, prompt  = result
            _ , new_bbox = self.mask_to_box(mask)

            if mask.mode != 'RGB':
                mask = mask.convert('RGB')
            location_array = np.where(np.array(mask) == (255,255,255), (0,0,0), location_array) 
